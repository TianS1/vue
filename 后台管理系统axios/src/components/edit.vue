<template>
  <div class="container">
    ...
    <router-link class="btn btn-default" :to="/table/+table.id">返回</router-link>
    <h1 class="page-header">编辑用户</h1>
    <form action="" v-on:submit="uppdataTable">
      <div class="well">
        <h4>用户信息</h4>
    ...
    methods:{
      ...
      uppdataTable(e){
        ...
        if(!this.table.name||!this.table.phone||!this.table.email){
          ...
          this.$http.put("http://localhost:3000/users/"+this.$route.params.id,uppdataTable)
            .then((responent)=>{
              //console.log(responent);
